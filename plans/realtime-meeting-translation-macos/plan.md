...
- 2026-07-18 Local spoken translation extension: the coordinated local plan now adds a main-route Cloud API/Local Whisper chooser and extends the implemented local text path with Gemma-backed translation, platform-native TTS buffers, and the existing selected-output CPAL playback. This plan remains the owner of shared capture/playback/lifecycle contracts and treats Google/OpenAI behavior as a regression baseline.
