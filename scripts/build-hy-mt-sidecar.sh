#!/usr/bin/env bash
# Build the HY-MT managed sidecar (PyInstaller one-folder) into
# sidecars/hy-mt/bundle/ so Tauri bundles it as a resource.
# macOS counterpart of the Windows release build — the sidecar must be
# built on the same OS as the desktop release.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SIDECAR_DIR="$ROOT/sidecars/hy-mt"
DIST_DIR="$SIDECAR_DIR/dist/hy-mt-sidecar"
BUNDLE_DIR="$SIDECAR_DIR/bundle"

if [ "$(uname -s)" != "Darwin" ]; then
  echo "Build the HY-MT sidecar on the same operating system as the desktop release." >&2
  exit 1
fi
if ! command -v uv >/dev/null 2>&1; then
  echo "The HY-MT sidecar build requires uv: https://docs.astral.sh/uv/" >&2
  exit 1
fi
...
EXECUTABLE="$DIST_DIR/hy-mt-sidecar"
...
case "$RESOLVED_BUNDLE/" in
  "$RESOLVED_SIDECAR"/*) ;;
  *)
    echo "Refusing to replace a bundle directory outside the HY-MT sidecar workspace." >&2
    exit 1
    ;;
esac
...
echo "HY-MT sidecar ready: $FILE_COUNT files, $SIZE_MIB MiB"
